...
Full-page composite component for reading comprehension, cloze tests, and annotated reading. Uses `layout-single` exclusively. **Must load `zone2-quiz-annotation/` (13 CSS) plus `audio-runtime.js` → `quiz-annotation-audio.js` → `zone2-quiz-annotation/` (17 JS modules in dependency order).** See `qa-test-all-types.html` for the exact `<link>` and `<script>` loading order.
...
**Must load `zone2-example-card.css`, `audio-runtime.js`, `example-card-audio.js`, and `example-card-core.js` → `example-card-authoring.js` → `example-card-student.js`.** For a stable editor contract, the 3 example-card modules should be loaded **after** `editor/editor-core.js`, `page-richtext-annotation-runtime.js`, and `doodle-runtime.js`.
